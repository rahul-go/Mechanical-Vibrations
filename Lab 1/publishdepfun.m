...
list=matlab.codetools.requiredFilesAndProducts(func);
out=list;
%% Sort to put top level file first
if ~strcmpi(func(end-1:end),'.m')
    funcTest = [func '.m'];                     % Name of top level .m file
else
    funcTest = func;
end
for i = 1:numel(out)                        % For exach element of list
    if regexpi(out{i},funcTest) ~= 0        % If function is top level function
        temp = out{1};                      % Save first entry to temp variable
        out{1} = out{i};                    % First list entry is top level script
        out{i} = temp;                      % Old location of top level script is replaced with previously first element
        break                               % Once top level script is found, don't bother testing rest
    end
end
%% Remove hits that are in matlabroot
out(strncmp(matlabroot,list,length(matlabroot)))=[];

%% Remove hits that are not .m files
for i = 1:length(out)
    [~,~,ext] = fileparts(out{i});
    index(i) = ~strcmpi(ext,'.m');
end
out(index) = [];
